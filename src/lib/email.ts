...
export async function sendSigningInvite(input: {
  ...
  const roleDisplay = (() => {
    ...
    if (!role) return '';
    if (/\bof\b/i.test(role)) return role;
    if (companyName) return `${role} of ${companyName}`;
    return role;
  })();
  const greetingLine = roleDisplay
...
<div style="font-family: ui-sans-serif,system-ui; line-height:1.6; font-size:14px; color:#111;">
  ...
  <div style="margin-top:18px; padding-top:12px; border-top:1px solid #e5e7eb; text-align:left;">
