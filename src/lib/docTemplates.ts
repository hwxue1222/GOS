...
export function renderRdrAuthorizationHtml(input: {
  ...
  representativeIdType?: 'NRIC' | 'PASSPORT' | string;
  matter?: string;
  ...
  const representativeIdTypeRaw = String(input.representativeIdType ?? '').trim();
  const representativeIdLabel = representativeIdTypeRaw === 'PASSPORT' ? 'Passport No.' : 'NRIC No.';
  const repSig = signatureLineBlocks({
  ...
  <body>
    ...
    <div class="grid2 block">
      <div>
        ...
        <div class="mt2">${representativeIdLabel}:&nbsp;&nbsp;${representativeIdNo}</div>
        <div class="mt2">Phone No.:&nbsp;&nbsp;</div>
...
<html>
  <head>
    ...
    <style>
      ...
      .issuer-footer { display: flex; gap: 12px; align-items: flex-start; }
      ...
      .pay-title { font-weight: 700; font-size: 13px; }
      .pay-list { margin: 8px 0 0 18px; padding: 0; font-size: 12px; }
      .pay-list li { margin: 2px 0; }
...
`.trim();
    ...
    <div class="box" style="margin-top: 14px;">
      ...
      <div class="box" style="margin-top: 16px; max-width: 560px;">
        <div class="pay-title">Payment Methods</div>
        ${input.issuer.paymentMethods?.length
          ? `<ul class="pay-list">${input.issuer.paymentMethods
