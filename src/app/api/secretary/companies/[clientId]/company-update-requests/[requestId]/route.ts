...
import { appendAuditLog, deleteCompanyUpdateRequest, deleteRejectedCompanyUpdateRequest, readDb } from '@/lib/db';
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ clientId: string; requestId: string }> }) {
  ...
  if (user.role === 'client') {
    ...
    const r = await deleteCompanyUpdateRequest({ requestId, deletedByUserId: user.id });
  ...
  await appendAuditLog({
    ...
    actorRole: user.role,
    area: 'secretary',
    action: 'delete_rejected_company_update_request',
    entityType: 'company_update_request',
    entityId: requestId,
    summary: `Delete rejected company update request: ${requestId}`,
  });
  return NextResponse.json({ ok: true });
}
