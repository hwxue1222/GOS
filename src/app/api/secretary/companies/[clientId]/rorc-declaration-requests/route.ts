...
export async function POST(req: Request, ctx: { params: Promise<{ clientId: string }> }) {
  ...
  const isClientUser = String(user.role) === 'client';
  ...
  if (!isClientUser) {
    ...
    if (!canViewSecretary && !(canProxy && isProxyingThisCompany)) {
  ...
  const allowCreate = isClientUser || (canProxy && isProxyingThisCompany);
  ...
  const controllerType = body?.controllerType ?? (modeRaw === 'PERSON' || modeRaw === 'COMPANY' ? (modeRaw as any) : undefined);
  const controllerPerson = (body?.controllerPerson ?? body?.person) as any;
  const controllerCompany = (body?.controllerCompany ?? body?.company) as any;
