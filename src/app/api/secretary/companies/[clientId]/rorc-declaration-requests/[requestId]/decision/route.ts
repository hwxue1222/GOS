...
import { appendAuditLog, decideRorcDeclarationRequest, readDb } from '@/lib/db';
...
export async function POST(req: Request, ctx: { params: Promise<{ clientId: string; requestId: string }> }) {
  ...
  const r = await decideRorcDeclarationRequest({
    requestId,
  ...
  await appendAuditLog({
    ...
    actorRole: user.role,
    area: 'secretary',
    action: `rorc_declaration_${decision.toLowerCase()}`,
    entityType: 'rorc_declaration_request',
    entityId: requestId,
    summary: `RORC declaration request ${decision}: ${requestId}`,
  });
