...
import { appendAuditLog, decideDirectorChangeRequest, readDb } from '@/lib/db';
...
export async function POST(req: Request, ctx: { params: Promise<{ clientId: string; requestId: string }> }) {
  ...
  const r = await decideDirectorChangeRequest({
    requestId,
  ...
  await appendAuditLog({
    ...
    actorRole: user.role,
    area: 'secretary',
    action: `director_change_${decision.toLowerCase()}`,
    entityType: 'director_change_request',
    entityId: requestId,
    summary: `Director change request ${decision}: ${requestId}`,
  });
