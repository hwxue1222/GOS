...
export default function InvoicesClient({ initialMe, initialInvoices, initialClients }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    const rows = invoices.filter((row) => {
      ...
      return textMatch(`${inv.invoiceNo} ${clientText} ${inv.currency} ${inv.status} ${row.createdByName}`, q);
    });
  ...
  async function reloadInvoices() {
    ...
    setInvoices(
      j.invoices.map((inv) => ({
        invoice: inv,
        client: clientById.get(inv.clientId) ?? null,
        createdByName: '-',
      })),
    );
  }
  ...
  async function addInvoice() {
    ...
    try {
      ...
      if (j?.invoice) {
        const client = clients.find((c) => c.id === j.invoice!.clientId) ?? null;
        setInvoices((prev) => [{ invoice: j.invoice!, client, createdByName: me.name }, ...prev]);
      }
      setShowAdd(false);
      resetNewInvoice();
    } finally {
  ...
  function exportCsv() {
    const header = ['Invoice No', 'Client Code', 'Client Name', 'Issue Date', 'Due Date', 'Currency', 'Total', 'Status'];
