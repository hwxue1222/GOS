...
export default function ContractNewClient({ initialTemplates }: Props) {
  ...
  const parseBulletBody = (rawInput: string) => {
    ...
    let rawNoNote = raw;
    let noteFromToken = '';
    if (rawNoNote.startsWith(`${NOTE_TOKEN}\n`)) {
      noteFromToken = rawNoNote.slice(`${NOTE_TOKEN}\n`.length);
      rawNoNote = '';
    } else {
      const marker = `\n${NOTE_TOKEN}\n`;
      const idx = rawNoNote.indexOf(marker);
      if (idx >= 0) {
        noteFromToken = rawNoNote.slice(idx + marker.length);
        rawNoNote = rawNoNote.slice(0, idx);
      }
    }

    const lines = rawNoNote.split('\n');
                          ...
                          <div className="md:col-span-1">
                            ...
                            <div className="mt-2">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setFields((prev) => ({ ...prev, [`service_price_${n}`]: e.target.value }))
                                }
                                className="mt-1 h-10 w-full px-3 rounded-lg border border-black/10 text-sm outline-none focus:ring-2 focus:ring-black/10"
                              />
                            </div>
